...
const parseBoundExp = (params: Sexp[]): Result<BoundExp> => 
    params.length !== 1 ? makeFailure("Expression not of the form (bounce <cexp>)") :
    !isIdentifier(params[0]) ? makeFailure("Expected string") :
    mapv(makeOk(makeVarRef(params[0])), (v: VarRef) => makeBoundExp(v));



const parseTimeExp = (params: Sexp[]): Result<TimeExp> => 
    params.length !== 1 ? makeFailure("Expression not of the form (time <exp>)") :
...
const unparseTimeExp = (c: CExp) : string =>
    `(time ${unparse(c)})`;
...
export const unparse = (exp: Parsed): string =>
    ...
    isBoundExp(exp)  ? `(bound? ${exp.var.var})` :
    //isTimeExp(exp)   ? `(time ${unparse(exp.exp)})` :
    isTimeExp(exp)   ? unparseTimeExp(exp.exp):
    exp;
