...
import { map, repeat, T, zipWith } from "ramda";
...
const evalBound = (exp: BoundExp, env: Env): Result<boolean> =>
    ...
    (applyEnv(env, exp.var.var).tag === "Failure")?
...
const evalTime = (exp: TimeExp, env: Env): Result<CompoundSExp> => 
{
    ...
    const ret = applicativeEval(exp.exp, env);
    const end = new Date().getTime();
    if(ret.tag === "Ok")
    {
        return makeOk(makeCompoundSExp(ret.value,end-start))
    }
    return makeFailure(`Bad procedure ${JSON.stringify(ret)}`);
}
    //isTrueValue(test) ? applicativeEval(exp.then, env) : 
    //applicativeEval(exp.alt, env));
