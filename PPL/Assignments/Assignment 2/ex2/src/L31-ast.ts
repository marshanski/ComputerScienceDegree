...
// L31
export type ElseClause = {tag: "ElseClause"; val: CExp[]; }
...
export const makeElseClause = (val: CExp[]) : ElseClause =>
    ({ tag: "ElseClause", val: val});
...
export const isCExp = (x: any): x is CExp =>
    isAtomicExp(x) || isCompoundExp(x)|| isCondExp(x);
...
export const parseCondExp = (params: Sexp[]): Result<CondExp> => {
    if (!allT(isCompoundSexp, params) ||params.length < 2 ) 
    {
        return makeFailure("Invalid cond clauses");
    }
    else
    {
        return bind(mapResult(parseCondClause,params.slice(0,-1)),(condclause:CondClause[]) => 
            bind(parseElseClause(params[params.length-1]),(elseclause:ElseClause)=> 
                makeOk(makeCondExp(condclause,elseclause))));
    }
} 
...
export const parseCondClause = (params: Sexp[]): Result<CondClause> => 
{
    ...
    {
        return makeFailure('Expression not of the form (if <cexp> <cexp>+)')
    }

    return mapv(mapResult(parseL31CExp,params),(cexp:CExp[]) => makeCondClause(cexp[0],rest(cexp)));
} 
...
export const parseElseClause = (params: Sexp[]): Result<ElseClause> => 
{
    ...
    {
        return makeFailure('Expression not of the form (if <else> <cexp>)')
    }
    return mapv(mapResult(parseL31CExp,params),(cexp:CExp[]) => 
        makeElseClause(cexp));
}   
...
const unparseCondExp = (c: CondExp) : string =>
    `(cond ${map((clause: CondClause) => `(${unparseL31(clause.test)} ${map(unparseL31, clause.then).join(" ")})`, c.condclauses).join(" ")} (${map(unparseL31, c.elseclause.val).join(" ")}))`
